...
int t1 = 10;                                                // The higher the number, the slower the timing.
int ledPins[] = { 5, 6, 7, 8, 9, 10, 11, 12, 2, 14};         // an array of pin numbers to which LEDs are attached
int pinCount = 10;                                           // the number of pins (i.e. the length of the array)

int barPins[] = { 15, 16, 17, 18, 19, 13};                   // an array of pin numbers to which LEDs are attached
int barCount = 6;
...
void setup(){
  ...
  for (int thisPin = 0; thisPin < barCount; thisPin++) {
    pinMode(barPins[thisPin], OUTPUT);
  }
...
displayTest();          // test display
  
ds1302_struct rtc;
  char buffer[80];     // the code uses 70 characters.

RESYNC:                // reload time from RTC every day          
...
CLOCK:                 // main loop 
  ...
  displaySeconds10(rtc.Seconds10);
  
  displayMinutes(rtc.Minutes);
  displayMinutes10(rtc.Minutes10);
  
  displayHour(rtc.h24.Hour);
  displayHour10(rtc.h24.Hour10);
